...
## 데이터 백업
`data/carlog.db` 파일 하나만 복사해 두시면 됩니다.

## 알려진 버그
- 연비 계산이 아직 불완전 합니다. 기존의 독점 서비스에서는 주유 단가와 주유비, 그리고 키로수만 입력하고 주유량(리터)을 입력하지 않아도 알아서 주유량을 계산하여 연비를 측정합니다. 그런데 지금은 이 기능이 구현되지 않았으며, 모든 것을 영수증 보면서 채워넣어야 하는 번거로움이 있습니다.
