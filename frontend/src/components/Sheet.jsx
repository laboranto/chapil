...
export default function Sheet({ children }) {
  ...
  useLayoutEffect(() => {
    ...
    // 튀어오른다. position:fixed + top:-y로 잠그면 있던 자리에 그대로 멎는다.
    //
    // 시트가 통통 튀던 건 이 잠금과 무관했다 — showModal()이 시트 안 첫 포커스
    // 요소를 잡으며 dialog를 스크롤한 탓이고, 고친 자리는 index.css의
    // .sheet-dialog { overflow: clip }이다.
